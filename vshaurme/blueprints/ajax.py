...
def notifications_count():
    if not current_user.is_authenticated:
        return jsonify(message=_l('Login required.')), 403
...
def collect(photo_id):
    ...
    if not current_user.confirmed:
        return jsonify(message=_l('Confirm account required.')), 400
    if not current_user.can('COLLECT'):
        return jsonify(message=_l('No permission.')), 403

    photo = Photo.query.get_or_404(photo_id)
    if current_user.is_collecting(photo):
        return jsonify(message=_l('Already collected.')), 400

    current_user.collect(photo)
    if current_user != photo.author and photo.author.receive_collect_notification:
        push_collect_notification(collector=current_user, photo_id=photo_id, receiver=photo.author)
    return jsonify(message=_l('Photo collected.'))
...
def uncollect(photo_id):
    ...
    if not current_user.is_collecting(photo):
        return jsonify(message=_l('Not collect yet.')), 400

    current_user.uncollect(photo)
    return jsonify(message=_l('Collect canceled.'))
...
def follow(username):
    ...
    if current_user.is_following(user):
        return jsonify(message=_l('Already followed.')), 400

    current_user.follow(user)
    if user.receive_collect_notification:
        push_follow_notification(follower=current_user, receiver=user)
    return jsonify(message=_l('User followed.'))
...
def unfollow(username):
    ...
    if not current_user.is_following(user):
        return jsonify(message=_l('Not follow yet.')), 400

    current_user.unfollow(user)
    return jsonify(message=_l('Follow canceled.'))
